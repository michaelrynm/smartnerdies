...
export default function Faq() {
  ...
  const fetchFaqData = async () => {
    try {
      ...
      console.log("FAQ RESPONSE", response.data.data);
  ...
  >
    {faqData.map((faq, index) => {
      ...
      return (
        ...
        >
          <AccordionTrigger className={`px-6 py-5 ${colorScheme.hover} transition-colors duration-200 text-left font-semibold text-gray-800 ${colorScheme.textHover}`}>
            {faq.Question}
          </AccordionTrigger>
          <AccordionContent className="px-6 pb-5 pt-2">
            <div className={`bg-white/70 rounded-xl p-4 border-l-4 ${colorScheme.accent}`}>
